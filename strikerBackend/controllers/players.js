...
exports.getAllPlayersDetials = (req, res, next) => {
    Player.find()
            ...
            const response = {
                ...
                players: docs.map(doc => {
                    return {
                        ...
                        request: {
                            type: 'GET',
                            url:'http://localhost:4200/players/' + doc._id
                        }
                    }
                })
            }
        ...
        .catch(err => {
            console.log(err);
                res.status(501).send({
                 message: 'Can\'t get all player details, Please try again later' 
            });
        });
}

//control mettod for get all player details
exports.getPlayersDetialsByName = (req, res, next) => {
    const shortName = req.params.Short_name;
    Player.find({ $text: { $search: shortName }})
        .select('_id Photo Short_name Long_name Age Height Weight Nationality Club Overall Potential Value Wage Preferred_Foot International_Reputation Weak_Foot Skill_Moves Position Jersey_Number Contract_Valid_Until Crossing Finishing Heading_Accuracy Short_Passing Volleys Dribbling Curve FK_Accuracy FK_Accuracy Long_Passing Ball_Control Acceleration Sprint_Speed Agility Reactions Balance Shot_Power Jumping Stamina Strength Long_Shots Aggression Interceptions Positioning Vision Penalties Composure Marking Standing_Tackle Sliding_Tackle GK_Diving GK_Handling GK_Kicking GK_Positioning GKReflexes')
        .exec()
        .then(docs => {
            const response = {
        ...
        .catch(err => {
            console.log(err);
                res.status(500).send({
                 message: 'Can\'t get player details by player name, Please try again later' 
            });
        });
}
        ...
        .then(result => {
            ...
            res.status(200).json({
                ...
                createdPlayer: {
                    ...
                    request: {
                            type: 'GET',
                            url:'http://localhost:4200/players/' + result._id
                    }
                }
...
exports.editPlayerDetails = (req, res, next) => {
    ...
    const player = new Player({
        ...
        const updatePlayer =  Player.findOneAndUpdate(id, player, {upsert:true, new: true, useFindAndModify:false})
        .exec()
        .then( result => {
            res.status(200).send({
...
exports.deletePlayerDetails = (req, res, next) => {
    ...
    .then(player => {
        ...
        if (player.length >= 1) {
                ...
                .then( result => {
                res.status(200).send({
                    message:'You are successfully deleted player details',
                    request: {
                        type: 'POST',
                        url:'http://localhost:4200/players/'+_id 
                    }
                });
            })
            .catch(err => {
